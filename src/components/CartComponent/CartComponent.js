...
const CartComponent = () => {
  ...
  // Why is total Important? Accumulates the subtotal dynamically.Starts from 0 and adds each item's total price.Ensures efficiency—avoiding extra loops. Works on any cart size, small or large. If we remove total, the calculation wouldn't work as reduce() needs an accumulator to store the running sum.

  //  Step	Current total	Current item	Calculation (total + item.price * item.quantity)	New total
  // 1	0 (initial)	{ price: 100, quantity: 2 }	0 + (100 * 2) = 200	200
  // 2	200	{ price: 150, quantity: 1 }	200 + (150 * 1) = 350	350

  const cartSubtotal = cart.reduce(
    //reduce() is used to add up the total cost of all items.
    (total, item) => total + item.price * item.quantity,
    0 //total starts at 0 (initial value). item.price * item.quantity calculates the total cost for each product.The result is stored in cartSubtotal.
  );
  ...
  const handleOrderPlacement = () => {
    ...
    triggerLoading();
      ...
      window.dispatchEvent(new Event("cartUpdated")); // Update cart event
    }, 7000);
  };
  return (
    <>
            ...
            <>
              ...
              {orderMessage && (
                <div className={style.orderMessage}>
                  Woohoo! Your order will be placed. We will send you a link for more
                  details like address, payment mode, etc., on your registered
                  email ID. <br/><span style={{color:"green"}}>You Can Also Update Your Order There.</span><br/>
                  <span style={{color:"red"}}>Update Your Email Id in My Account</span>
                </div>
              )}
            </>
          )}
